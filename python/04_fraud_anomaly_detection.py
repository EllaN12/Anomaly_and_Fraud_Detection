"""
LEARNING LAB 17 (Python port) -- MAIN FRAUD / ANOMALY DETECTION
===============================================================
METHOD : scikit-learn IsolationForest  (replaces h2o.isolationForest)
CASE   : Credit Card Fraud Detection   (creditcard.csv)

Faithfully mirrors lab_17_credit_fraud_anomaly_detection.R, extended
with explicit class-imbalance handling.

WHY IMBALANCE MATTERS
---------------------
The dataset is 577:1  (99.83% non-fraud, 0.17% fraud = 492 / 284,807).
This breaks three things the original code took for granted:
  ...
  2. Fixed 99th-percentile threshold
         Flags 1% ≈ 2,849 rows.  The real fraud count is 492 (0.17%).
         We compare three thresholds side-by-side:
           (a) 99th-percentile  (original R lab)
...
Mirrors R lab sections 1–6, then adds §7 (Python-only imbalance section).
...
def load_data() -> pd.DataFrame:
    """R: credit_card_tbl <- vroom("data/creditcard.csv")"""
...
def fit_iforest(
    ...
    """
    R:
      isoforest <- h2o.isolationForest(training_frame = credit_card_h2o,
                       x = predictors, ntrees = 100, seed = 1234)
...
def predict_scores(
    ...
    """
    R: predictions <- predict(isoforest, newdata = credit_card_h2o)

    Flip sign: sklearn score_samples is higher = more normal;
    we return higher = more anomalous, matching h2o's `predict`.
    """
...
def threshold_fixed_pct(scores: np.ndarray, q: float = 0.99) -> tuple[np.ndarray, float]:
    """Original R lab: flag top 1% (99th-percentile)."""
...
def plot_roc(y: np.ndarray, scores: np.ndarray, path: Path) -> float:
    """
    R: roc_curve + roc_auc(class, predict)
...
def iso_forest_one_seed(
    ...
    """R (§6.1): iso_forest <- function(seed) { h2o.isolationForest(...) }"""
    model = IsolationForest(
        n_estimators=100,
        max_samples="auto",
...
def stabilized_scores(
    ...
    """R (§6.2–6.3): map seeds → mean predictions."""
